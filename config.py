...
MENTION_MATCH_WITHOUT_AT = False  # GROUP_MESSAGE_CREATE：群消息未@机器人时是否仍匹配插件正则触发，默认关闭
MENTION_WHITELIST_GROUPS = []  # GROUP_MESSAGE_CREATE白名单：白名单内的群即使未@机器人也可触发插件（仅MENTION_MATCH_WITHOUT_AT关闭时生效）
PROCESS_BOT_MESSAGE = False  # 是否处理其他机器人发送的消息，关闭后自动忽略bot消息避免循环
